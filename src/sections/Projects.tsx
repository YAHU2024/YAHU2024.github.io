...
const statusLabel: Record<ProjectStatus, string> = {
  doing: copy.projects.status.doing,
  shipped: copy.projects.status.shipped,
  idea: copy.projects.status.idea,
}
...
function StatusBadge({ status }: { status: ProjectStatus }) {
  ...
  if (status === 'shipped') {
    return (
      <span className={`${base} bg-accent text-[var(--accent-ink)]`}>{statusLabel.shipped}</span>
    )
  }
  if (status === 'doing') {
    return (
...
function ProjectCard({ p }: { p: ProjectItem }) {
  return (
    <GlassCard className="h-full p-7" tilt>
      ...
      <h3 className="gc-z2 text-xl font-extrabold">{p.name}</h3>
      <p className="gc-z3 mt-2.5 flex-1 text-[0.95rem] leading-relaxed text-muted">{p.oneLine}</p>

      {typeof p.progress === 'number' && (
        <div className="gc-z3 mt-4">
          <div className="flex items-baseline justify-between text-xs font-bold text-muted">
            <span>{copy.projects.progress}</span>
          ...
          <div
            ...
            aria-valuemin={0}
            aria-valuemax={100}
            aria-label={`${p.name} ${copy.projects.progress}`}
          >
      ...
      {p.note && <p className="gc-z3 mt-3 text-xs leading-relaxed text-muted">{p.note}</p>}
      ...
      {(p.links?.repo || p.links?.site) && (
        <div className="gc-z2 mt-5 flex flex-wrap items-center gap-x-4 gap-y-1">
          {p.links?.repo && (
            ...
            >
              {copy.projects.repo} <ArrowUpRight className="ar-pop h-3.5 w-3.5" />
            </a>
          )}
          {p.links?.site && (
            <a
              ...
              className="inline-flex items-center gap-1 text-sm font-extrabold text-accent hover:underline"
            >
              {copy.projects.site} <ArrowUpRight className="ar-pop h-3.5 w-3.5" />
            </a>
          )}
        </div>
      )}
...
function PlaceholderCard() {
  return (
    <GlassCard className="placeholder-card h-full p-7">
      ...
      <h3 className="gc-z2 text-xl font-extrabold text-muted">{copy.projects.placeholderTitle}</h3>
      <p className="gc-z3 mt-2.5 flex-1 text-[0.95rem] leading-relaxed text-muted">
        {copy.projects.placeholderText}
      </p>
    </GlassCard>
  )
}
        ...
        <Reveal>
          <div className="mb-9 flex items-baseline justify-between gap-4">
            <h2 className="text-3xl font-black">
              <span className="mr-3 text-base font-bold text-accent">{copy.projects.num}</span>
              {copy.projects.title}
            </h2>
            <a
            ...
            >
              {copy.projects.allOnGitHub(repoCount)} <span className="ar-pop">→</span>
            </a>
          </div>
        </Reveal>
            ...
            <div key={status} className="mt-10 first:mt-0">
              <Reveal>
                <div
                  ...
                  aria-label={copy.projects.groupAria(label, list.length)}
                >
